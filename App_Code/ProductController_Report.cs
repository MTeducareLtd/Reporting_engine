...
namespace ShoppingCart.BL
{
    public class ProductController_Report
    {
        ...
        public static DataSet GetStudentDetailsforPDF(string SBEntryCode)
        {
            SqlParameter p = new SqlParameter("@SBEntrycode", SqlDbType.VarChar);
            p.Value = SBEntryCode;
            return (SqlHelper.ExecuteDataset(ConnectionString.GetConnectionString(), CommandType.StoredProcedure, "USP_GET_STUDENT_DETAILS_FOR_PDF", p));
        }
    }
}
